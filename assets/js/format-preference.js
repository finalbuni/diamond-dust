/* Homepage navigation only. Never redirect a directly opened chapter or section URL. */
(() => {
  let format;
  try { format = localStorage.getItem('diamond-dust-reading-format'); } catch (_) {}
  if (format !== 'short') return;
  document.querySelectorAll('.chapter-list-item[data-short-url]').forEach(link => {
